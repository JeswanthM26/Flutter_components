            ...
            Expanded(
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(style.borderRadius),
                  border: Border.all(
                    color: widget.mainFocusNode.hasFocus
                        ? AppzStyleConfig.instance.getStyleForState(AppzFieldState.focused).borderColor
                        : style.borderColor,
...
class _MobileInputWidgetState extends State<MobileInputWidget> {
  ...
  void _validate(String? val) {
    ...
    if (widget.validationType == AppzInputValidationType.mandatory &&
        digitsOnly.isEmpty) {
      error = 'This field is required.';
    } else if (digitsOnly.isNotEmpty && !RegExp(r'^\d{10}$').hasMatch(digitsOnly)) {
      error = 'Mobile number must be 10 digits.';
    }
